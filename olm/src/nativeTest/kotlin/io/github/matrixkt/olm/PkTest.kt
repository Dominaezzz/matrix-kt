...
class PkTest {
    ...
    @Test
    fun test03Signing() {
        val signing = PkSigning()
        val seed = Random.nextBytes(PkSigning.seedLength.toInt()) // Check this PkSigning.generateSeed()

        assertEquals(PkSigning.seedLength, seed.size.toLong())
        val pubkey = signing.fromSeed(seed)
        ...
        val utility = Utility()
        utility.verifyEd25519Signature(pubkey, message, signature)
